...
bool QWinRTServices::openUrl(const QUrl &url)
{
    ...
    do { qGuiApp->processEvents(); } while (launchOp->GetResults(&result) == E_ILLEGAL_METHOD_CALL);
    launchOp->Release();
...
bool QWinRTServices::openDocument(const QUrl &url)
{
    ...
    do { qGuiApp->processEvents(); } while (fileOp->GetResults(&file) == E_ILLEGAL_METHOD_CALL);
    fileOp->Release();
    if (!file)
        return false;
